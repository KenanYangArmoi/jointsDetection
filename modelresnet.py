...
class Model:
    ...
    def inference(self, data, is_training):
        ...
        with tf.variable_scope('conv2_1') as scope:
            with tf.variable_scope('shortcut1') as scope:
        ...
        with tf.variable_scope('conv3_1') as scope:
            with tf.variable_scope('shortcut2') as scope:
        ...
        with tf.variable_scope('conv4_1') as scope:
            with tf.variable_scope('shortcut3') as scope:
        ...
        with tf.variable_scope('conv5_1') as scope:
            with tf.variable_scope('shortcut4') as scope:
    ...
    def loss(self, logits, labels, marks, Pweights, batch_size):
        # logits_pxy in the shape of [[p0,x0,y0],
        #                            [p1,x1, y1],
        #                              ...
        #                            [p13,x13,y13]] in each batch cell
        with tf.variable_scope('loss') as scope:
            logits_bpxy = tf.reshape(logits, [batch_size, 14, 3])
            ...
            loss_xy = tf.losses.mean_squared_error(labels, logits_xy, marks)
            # make the logits_marks go through sigmoid function
            # tf.nn.sigmoid_cross_entropy_with_logits
            loss_marks = tf.losses.mean_squared_error(marks, tf.sigmoid(logits_marks), Pweights)
            loss = tf.add(loss_xy, loss_marks, name='loss')
    ...
    #         mark_diff_abs = tf.losses.absolute_difference(marks, logits_marks_sigmoid)
    #         for batch in sess.run(mark_diff_abs):
    #             for value in batch:
    #                 total_mark = total_mark +1
    #                 if value < 0.1:
    #                     correct_mark = correct_mark +1
    #         mark_accuracy = correct_mark/total_mark
    #
    #
    #
    ...
    #         distance_accuracy = tf.add(accuracy, 0, name='accuracy')
    #         tf.summary.scalar(scope.name + '/accuracy', accuracy)
    #
    #     return mark_accuracy, distance_accuracy
